...
if [ "`grep_prop tsra.game $OPTIONALS`" != 0 ]; then
  sed -i 's|#p||g' $FILE
  sed -i 's|#g||g' $FILE
else
  ui_print "- Does not use 360 Reality Audio game patch &"
  ui_print "  rerouting stream"
  ui_print " "
fi
...
# overlay
if [ -d $MODPATH/system/product/overlay ]\
&& [ ! -d /product/overlay ]; then
